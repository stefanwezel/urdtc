...
The recent surge in popularity of machine learning methods has made its ambivalence obvious. Public opinion is split between hype and mistrust. 
%TODO examples for where ML did something bad
Several cases where an artificial intelligence made severe mistakes have made it clear that, despite being deployed more frequently than ever before, machine learning has yet to prove itself.
...
%TODO overconfidence of softmax and uncertainty
% TODO more motivation on uncertainty
Uncertainty information is valuable. Especially in real-world scenarios, where we might encounter only limited, or noisy data to learn from. Where prediction, far into the future are required. Or, where decisions with high consequences are made.\\
Unfortunately, uncertainty in classification models can not simply be measured by model outputs. Typical architectures use a softmax layer, to turn model outputs into a probability density function. The input for the softmax (logits) can be viewed as a predictive mean of the (citation needed) of the models output given data. As a point estimate, this predictive mean lacks variance information, which would be the basis for uncertainty. The output of a softmax layer may be overconfident for highly uncertain classes. 
%TODO think about whether this is true
Let's consider logits, passed through a softmax that are centered closely around $0$. These logits are probably from data, a model is not certain about. We can consider a logit value of zero as $p=0.5$ for this class being true. Thus most classes have the probability of a coin flip. Another logit, a scaled version of the first one should result in a more confident output. However, if passed through softmax, both vectors result a similar output.
...
Such a model may be applied in settings, where a high confidence classification for out-of-domain examples may have severe consequences. For example in medical settings or in autonomous driving. There, in cases of high uncertainty, a human could be notified and consulted, preventing the model from making a (potentially wrong) high-risk decision. The model would still provide a decision tree as explanation, which could be utilized by a human user to make a final decision.
...
\subsection{Learning}
Ideally, the model should come to a classification in as few communication steps as possible. The tree resulting from few communication steps is shallow and easier to interpret. To keep the model from using deep trees, the classification loss is divided by the number of time steps (communication steps):
\begin{align}
