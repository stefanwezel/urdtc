...
After training our model on the proposed split, we investigate uncertainties of classes that contain unseen attributes (Figure \ref{fig:unseen_attr_classes}). We consider the mean uncertainty per example.
%For (2), we consider all uncertainty values of each attribute.\\
We observe that the 24 classes, possessing unseen attributes have an overall higher uncertainty. Figure \ref{fig:unseen_attr_classes} displays the modeled distribution of mean uncertainty per example, divided into classes that only posses attributes, occurring in the training data, and classes that possess 5 attributes, never encountered during training. We observe, that the latter have an overall higher uncertainty and a much wider variance. While the uncertainty values from seen attribute classes are concentrated, the uncertainty values of unseen attribute classes have a large spread.
